...
def get_ela_from_pil(original_image: Image.Image, quality: int = 95) -> Image.Image:
    buffer = io.BytesIO()
...
def read_image():
    ...
    for path in file_list:
        for i, filename in enumerate(os.listdir(path)):
            try:
                original_pil = Image.open(path + filename).convert('RGB')
                ela_pil = get_ela_from_pil(original_pil)

                original_tensor = data_transforms(original_pil)
                ela_tensor = data_transforms(ela_pil)

                image_id = filename.split('/')[-1].split(".")[0].lower()
                image_dict['original'][image_id] = original_tensor
                image_dict['ela'][image_id] = ela_tensor
...
def write_data(flag, image_dict, text_only):
    def read_post(flag):
    ...
    def paired(text_only=False):
        ...
        # 获取原始图和ELA图的字典
        original_images = image_dict['original']
        ela_images = image_dict['ela']

        for i, id in enumerate(post['post_id']):
            for image_id in post.iloc[i]['image_id'].split('|'):
                image_id = image_id.split("/")[-1].split(".")[0]
                if image_id in original_images:  # 检查图片是否存在
                    break

            # 如果图片存在于我们的字典中
            if text_only or image_id in original_images:
                if not text_only:
                    image_id_list.append(image_id)
                    ordered_image.append(original_images[image_id])
                    ordered_ela_image.append(ela_images[image_id])  # <--- 添加对应的ELA张量
        ...
        data = {
            ...
            "image": ordered_image,
            "ela_image": ordered_ela_image,  # <--- 新增ela_image键
            "label": np.array(label, dtype=int),
            "event_label": np.array(ordered_event, dtype=int),
